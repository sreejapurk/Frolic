...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Prefer rawDate (ISO format) over formatted date string
    const date = firstSlot.rawDate || firstSlot.date || data.date || ''
    ...
    for (const slot of slots) {
      const slotDate = slot.rawDate || slot.date
      if (!slotDate || !slot.time) continue
      ...
      await query(
        ...
        [id, slotDate, slot.time, slot.duration || '60 min', slotSpots]
      )
    }

    return NextResponse.json({ ok: true, savedDate: date })
  } catch (err: any) {
